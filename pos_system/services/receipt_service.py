...
<div class="receipt">
  ...
  <div class="company-info">
    {(company.get('street') or '') and company.get('street') + '<br>'}
    {(company.get('city') or '') and company.get('city')}
    {(company.get('state') or '') and ', ' + company.get('state')}
    {(company.get('zip_code') or '') and ' ' + company.get('zip_code')}
    {(company.get('country') or '') and '<br>' + company.get('country')}
    {(company.get('phone') or '') and '<br>' + _('tel', lang) + ': ' + company.get('phone')}
    {(company.get('email') or '') and '<br>' + company.get('email')}
    {(company.get('vat') or '') and '<br>' + _('tax_id', lang) + ': ' + company.get('vat')}
  </div>
  ...
  <table class="payment-info">
    ...
    {(total - paid) > 0.01 and f'<tr style="font-weight:bold;color:#c0392b"><td>{_("remaining", lang)}:</td><td style="text-align:right">{currency_format(total - paid, currency)}</td></tr>' or ''}
  </table>
...
class ReceiptPDF(FPDF):
    def __init__(self):
        super().__init__(orientation='P', unit='mm', format=(58, 297))
...
def generate_receipt_pdf(order_id, lang='en'):
    ...
    if (total - paid) > 0.01:
        pdf.set_font('Courier', 'B', 9)
        pdf.set_x(left)
        pdf.cell(page_w - 28, 4, f"{_('remaining', lang)}:", align='R')
        pdf.cell(28, 4, currency_format(total - paid, currency), align='R')
        pdf.ln(4)
