...
def generate_receipt_html(order_id, lang='en'):
    ...
    paid_row = f'<tr><td>{_("paid", lang)}:</td><td style="text-align:right">{currency_format(paid, currency)}</td></tr>'
    remaining_row = f'<tr style="font-weight:bold"><td>{_("remaining", lang)}:</td><td style="text-align:right">{currency_format(remaining, currency)}</td></tr>' if remaining > 0.01 else ''
...
def generate_receipt_pdf(order_id, lang='en'):
    ...
    pdf.set_x(left)
    pdf.cell(page_w - 20, 4, f"{_('paid', lang)}:", align='R')
    pdf.cell(20, 4, currency_format(paid, currency), align='R')
    pdf.ln(4)

    if remaining > 0.01:
        pdf.set_font('Courier', 'B', 8)
        pdf.set_x(left)
        pdf.cell(page_w - 20, 4, f"{_('remaining', lang)}:", align='R')
        pdf.cell(20, 4, currency_format(remaining, currency), align='R')
        pdf.ln(4)
        pdf.set_font('Courier', '', 8)
